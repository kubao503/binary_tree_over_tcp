...
use errors::{Result, TreeCreationError};
...
impl TreeCreator {
    ...
    pub fn add_node<'a>(&mut self, node_data: NodeData) -> Result<()> {
    ...
    fn take_node(&mut self, index: usize) -> Result<Node> {
        let node = self
            .nodes
            .get_mut(index)
    ...
    fn validate_tree(&mut self) -> Result<()> {
